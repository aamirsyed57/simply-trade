...
async def create_assignment(data: AssignmentCreate, db: AsyncSession = Depends(get_db)):
    ...
    # Validate allocation does not exceed available cash
    if data.allocation > portfolio.cash_available:
        raise HTTPException(
            status_code=status.HTTP_422_UNPROCESSABLE_ENTITY,
            detail=(
                f"Allocation {data.allocation} exceeds available cash {portfolio.cash_available}"
            ),
        )
